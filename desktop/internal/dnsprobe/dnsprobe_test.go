...
func TestProbeNoEDNS(t *testing.T) {
	...
	// The penalty must be applied: a perfect-latency no-EDNS resolver
	// scores exactly 100-10 = 90; sub-ms local UDP on Windows hits that
	// ceiling, so >90 means the penalty was skipped.
	if result.Score > 90.0+1e-9 {
		t.Fatalf("no-EDNS resolver scored %.1f — expected the 10-point penalty", result.Score)
	}
}
