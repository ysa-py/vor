// Shield Firefox Extension Popup Controller
// Same functionality as Chrome but using browser.* APIs
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    connectBtn.addEventListener('click', () => {
        if (isConnected) {
            api.runtime.sendMessage({ type: 'disconnect' }, (response) => {
                updateUI({ connected: false });
            });
        } else {
            const server = serverSelect.value;
            api.runtime.sendMessage({ type: 'connect', server }, (response) => {
    ...
    document.querySelectorAll('.toggle').forEach(toggle => {
        toggle.addEventListener('click', () => {
            ...
            api.runtime.sendMessage({ type: 'setSetting', setting, enabled });
        });
    });
    ...
    serverSelect.addEventListener('change', () => {
        if (isConnected) {
            api.runtime.sendMessage({ type: 'reconnect', server: serverSelect.value });
        }
    });
