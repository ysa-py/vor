/**
 * UnifiedShield Alibaba Cloud Relay Logic
 *
 * Handles WebSocket upgrades, frame multiplexing, connection state
 * management, and upstream proxy connections.
 */
...
export class RelayHandler {
  ...
  constructor(privateKey: Uint8Array, obfuscator: Obfuscator, rateLimiter: RateLimiter) {
  ...
  async relayToUpstream(
    ...
    const paddedPayload = this.obfuscator.addPadding(payload);
    ...
    return new Promise((resolve, reject) => {
      const options: http.RequestOptions = {
        ...
        headers: {
          ...
          'User-Agent': this.obfuscator.getRandomUserAgent(),
          'X-Forwarded-For': this.obfuscator.generateFakeXFF(),
        },
        timeout: 30000,
      };
      ...
      const req = http.request(options, (res) => {
        ...
        res.on('end', () => {
          ...
          const depaddedData = this.obfuscator.removePadding(new Uint8Array(responseData));
          resolve(depaddedData);
        });
      ...
      req.write(Buffer.from(paddedPayload));
      req.end();
    });
  }
  ...
  closeConnection(id: string, reason: string = 'normal'): void {
    ...
    if (conn) {
      ...
      if (conn.upstreamSocket) {
        try {
          conn.upstreamSocket.destroy();
        } catch {}
        conn.upstreamSocket = null;
      }
  ...
  getConnectionStats(): { total: number; active: number; byState: Record<string, number> } {
